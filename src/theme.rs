...
///
/// **아주 옅어야 한다.** 탭은 데이터가 아니라 구조이고, 들여쓰기가 많은 파일은
/// 화면의 상당 부분이 탭이다 — 조금만 진해도 그 줄무늬가 본문보다 먼저 눈에
/// 들어온다. 개행 기호(`line_ending_fg`)를 한 단계 물러나게 한 것과 같은 판단이다.
///
/// 찾기 음영(`find_match_bg`, 알파 64)보다 옅게 두어, 탭 위에 매치가 겹쳐도
/// 매치 쪽이 이긴다.
///
/// **알파 값은 화면에서 맞춘 것이다.** 처음에 28로 뒀더니 순백 위에서
/// rgb(240,242,246) — 흰색과 9단계 차이라 실제로는 보이지 않았다(줄무늬
/// `stripe_bg`(gray 250)보다 조금 진한 정도다). 56이면 약 rgb(223,229,237)로
/// 격자선(gray 216)과 비슷한 무게라, 있는 줄은 알아보되 글자를 방해하지 않는다.
/// 이 값을 낮출 때는 흰 배경에서 실제로 보이는지 확인할 것.
pub fn tab_bg() -> Color32 {
    Color32::from_rgba_unmultiplied(110, 135, 175, 56)
}

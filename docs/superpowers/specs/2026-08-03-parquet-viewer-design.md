...
행 수는 `ParquetDoc`이 답한다.

`doc_line_count`가 Parquet 문서에서 `parquet.total_rows()`를 돌려주도록 분기한다.
...
전체를 `"`로 감싸고 내부 `"`는 `""`로 바꾼다. `split_fields`가 되돌릴 수 있는
형태여야 한다.
...
**정렬 순열은 데이터 행 기준이다.** `SortState.permutation`은 텍스트 경로에서도
헤더를 제외한 데이터 행의 순열이므로 규약이 같다 — 길이는 `total_rows`다.
